...
public class ConvertableUtil <Key> implements Convertable <Key> {

    public static <Key, V extends ConvertableBook> Map convert(List <? extends Convertable <Key> > list) {

        Map<Key, Convertable<Key>> result = new HashMap();

        for(Convertable<Key> l: list){
...
значениями которого являются элементы переданного cписка, а ключами являются объекты, полученные вызовом интерфейсного метода getKey.
...
3. В множестве ключей словаря возвращенного методом convert класса ConvertableUtil должны содержаться ключи конвертируемого списка,
полученные с помощью вызова метода getKey на каждом объекте списка.
...
/*public class ConvertableUtil <T extends Convertable> {

    public static <T> Map convert(List<T> list) {
