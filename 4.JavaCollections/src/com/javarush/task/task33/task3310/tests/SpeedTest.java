...
public class SpeedTest {
    ...
    public long getTimeToGetIds(Shortener shortener, Set<String> strings, Set<Long> id) {
...
//        System.out.println(msDelayGetString);
    ...
    public void testHashMapStorage (){
        ...
        float expected = getTimeToGetIds(shortener1,origStrings,ids);
        float actual = getTimeToGetIds(shortener2,origStrings,ids);
        Assert.assertTrue("HashMapStorageStrategy медленнее HashBiMapStorageStrategy",expected > actual);

        float expected2 = getTimeToGetStrings(shortener1,ids,actualStrings);
        float actual2 = getTimeToGetStrings(shortener2,ids,actualStrings);
        float delta = 30;
        Assert.assertEquals("разница меньше delta = 30 ",expected2, actual2, delta);
    }
}

/*
Напишем еще один тест, который проверит, что получить идентификатор для строки используя стратегию HashBiMapStorageStrategy можно быстрее,
...
Он должен возвращать время в миллисекундах необходимое для получения строк для всех идентификаторов из ids. Строки должны быть записаны в strings.

 15.4. Добавь в класс SpeedTest тест testHashMapStorage(). Он должен:
15.4.1. Создавать два объекта типа Shortener, один на базе HashMapStorageStrategy, второй на базе HashBiMapStorageStrategy.
...
3. В методе testHashMapStorage должно быть выполнено сравнение времени получения множества ключей и множества значений
